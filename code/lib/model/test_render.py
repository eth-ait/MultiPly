...
if __name__ == '__main__':
    # inputs is from the Hi4DDataset in Hi4D.py
    img_size = inputs["img_size"]
    P = inputs["P"][0].cpu().numpy()
    P_norm = np.eye(4)
    P_norm[:, :] = P[:, :]
    #  assert the scale is the same for all the frames
    ...
    # verts_list is from unscaled deformed mesh after marching cube
